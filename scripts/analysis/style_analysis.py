...
def select_successful_style(dir,required_artist,threshold=None,top_k=10):
    ...
    # Check whether this is a TINA attack result (log contains optimized_noise_path).
    is_tina = False
    if len(data) > 1 and isinstance(data[1], dict):
        is_tina = 'optimized_noise_path' in data[1]
    ...
    if len(filtered_entries) > 0:
        # Check whether style information exists
        if 'style' in filtered_entries[0] and isinstance(filtered_entries[0]['style'][0],list):
            sorted_filtered_entries = sorted(
                filtered_entries,
                key=lambda x: [item['score'] for item in x['style'] if item['label'] == required_artist][0] if 'style' in x and any(item['label'] == required_artist for item in x['style']) else 0,
                reverse=True
            )
        elif 'style' in filtered_entries[0]:
            sorted_filtered_entries = sorted(
                filtered_entries,
                key=lambda x: [item['score'] for item in x['style'] if item['label'] == required_artist][0] if 'style' in x and any(item['label'] == required_artist for item in x['style']) else 0,
                reverse=True
            ...
            # If style information is missing, sort by attack_score
            sorted_filtered_entries = sorted(
                filtered_entries,
    ...
    if len(sorted_filtered_entries) > 0:
        ...
        # If this is a TINA attack, check whether reconstructed image exists
        if is_tina:
            reconstructed_path = os.path.join(image_dir, 'reconstructed.png')
            if os.path.exists(reconstructed_path):
                shutil.copy(reconstructed_path, os.path.join(new_dir, 'reconstructed.png'))
