...
Section('overall', 'Overall configs').params(
    ...
    attacker = Param(OneOf(['gcg', 'text_grad', 'tina', 'hard_prompt', 'hard_prompt_multi','random', 'seed_search','no_attack']), required=True, desc='Attack algorithm'),
...
Section('attacker.tina', 'TINA').enable_if(
    lambda cfg: cfg['overall.attacker'] == 'tina'
).params(
    lr = Param(float, default=0.001, desc='Learning rate'),
    opt_round = Param(int, default=25, desc='Number of optimization rounds'),
    num_ddim_steps = Param(int, default=50, desc='Number of DDIM steps'),
)
