...
def generate_images(
    ...
    '''
    vae = AutoencoderKL.from_pretrained(model_name_or_path, subfolder="vae", cache_dir=cache_dir)
    tokenizer = CLIPTokenizer.from_pretrained(model_name_or_path, subfolder="tokenizer", cache_dir=cache_dir)
    text_encoder = CLIPTextModel.from_pretrained(model_name_or_path, subfolder="text_encoder", cache_dir=cache_dir)
    unet = UNet2DConditionModel.from_pretrained(model_name_or_path, subfolder="unet", cache_dir=cache_dir)
...
if __name__=='__main__':
    ...
    parser.add_argument(
        '--model_name_or_path',
        help='Diffusers pretrained model id or local directory (default: CompVis/stable-diffusion-v1-4)',
        type=str,
        required=False,
