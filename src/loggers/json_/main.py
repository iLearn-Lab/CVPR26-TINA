...
class JSONLogger(BaseLogger):
    ...
    def save_noise(self, name, noise):
        """Save optimized noise tensor.
        
        Args:
            name: Name of the noise file.
            noise: Noise tensor to save (torch.Tensor).
        
        Returns:
            str: Saved noise file path.
        """
        # Create a dedicated directory for saved noise tensors.
        noise_root = os.path.join(os.path.dirname(self.img_root), 'noises')
        os.makedirs(noise_root, exist_ok=True)
        
        # Save noise data.
        path = os.path.join(noise_root, f'{name}.pt')
        torch.save(noise, path)
        return path  # Return path so callers can log it.
